...
%{$reset_color%}# "
        ...
        PROMPT="%{$reset_color%}%{$fg[red]%}$USER%{$reset_color%}@%{$fg[red]%}%m%{$reset_color%}$PROMPT"
        ;;
    *)
        PROMPT="
%{$reset_color%}$ "
        PROMPT="%{$reset_color%}[%{$fg[grey]%}%B%~%b%{$reset_color%}] %1(v|%F{blue}%1v%f|)$PROMPT"
        PROMPT="%{$reset_color%}%{$fg[green]%}$USER%{$reset_color%}@%{$fg[red]%}%m%{$reset_color%}$PROMPT"
        ;;
esac
...
alias updagrade="\
    eix-sync -q;\
    emerge -qtauDN --keep-going --with-bdeps=y @world;\
    dispatch-conf;\
    emerge -q1 @preserved-rebuild;\
    dispatch-conf;\
    eselect news read;"
